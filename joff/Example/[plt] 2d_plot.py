...
#cmap: Blues viridis jet cool
color=plt.get_cmap('cool')(np.linspace(0.3, 1, 5))
# color=plt.get_cmap('cool')(np.linspace(0.1, 0.9, 5))
linestyles = ['-', '--', '-.', ':', '-']
# markers = ['o', '^', 's', '*', 'D']

for i in range(5):
    legend = '$δ = {}$'.format(yy[i])
    ax.plot(xx, Z.T[:,i], linewidth = 3, c = color[i], label = legend, linestyle=linestyles[i], markersize=6)
    # ax.plot(xx, Z.T[:, i], linewidth=3, c=color[i], label=legend, marker=markers[i])
...
lgd = ax.legend(loc = 'upper right', fontsize=58, frameon = False)
lgd.get_frame().set_alpha(0.5)


plt.tight_layout()
plt.savefig( 'img/N_min_C_D.pdf', bbox_inches='tight')
plt.show()
plt.close(fig)
